...
public:
  ...
  void execute() {
    int start = (this->id * (this->N/this->ntasks)) + 1;
    int end = (this->id == this->ntasks - 1) ? this->N-1 : (this->id + 1) * (this->N/this->ntasks) - 1;
...
int main(int argc, char* argv[]) {
  ...
  int ntasks = (argc == 14) ? std::stoi(argv[13]) :  1;
  // std::cerr << ntasks << "\n";
  ...
  pthread_barrier_init(&bar, nullptr, ntasks);
